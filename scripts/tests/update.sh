...
function tests_update_system()
{
  ...
  then
    ...
    run_verbose sudo apt-get -qq install --yes m4
  elif [[ ${image_name} == *ubuntu* ]] || [[ ${image_name} == *debian* ]]
  then
    run_verbose apt-get -qq install --yes m4
  elif [[ ${image_name} == *raspbian* ]]
  then
    run_verbose apt-get -qq install --yes m4
  elif [[ ${image_name} == *centos* ]] || [[ ${image_name} == *redhat* ]] || [[ ${image_name} == *fedora* ]]
  then
    run_verbose yum install --assumeyes --quiet gm4
  elif [[ ${image_name} == *suse* ]]
  then
    run_verbose zypper --quiet --no-gpg-checks install --no-confirm m4
  elif [[ ${image_name} == *manjaro* ]]
  then
    run_verbose pacman -S --quiet --noconfirm --noprogressbar m4
