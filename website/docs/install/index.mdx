...
title: How to install the xPack GNU bison binaries
...
The **xPack GNU bison** can be installed automatically, using the `xpm` command
(the recommended method), or manually, by downloading and unpacking one of the
portable archives.

## Automated install

The easiest (and recommended) way to install GNU bison
is by using the **binary xPack**, available
as <a href={ `https://www.npmjs.com/package/@xpack-dev-tools/bison` }><code>@xpack-dev-tools/bison</code></a> from
...
<code>xpacks/@xpack-dev-tools/bison</code>.
...
**bison** package into the project:
...
* identify the platform specific archive for the latest available version
  of GNU bison, download it into a cache and unpack
...
* create a local symbolic link
  like <code>xpacks/@xpack-dev-tools/bison</code> pointing to the
...
* add links/forwarders into
  ...
  <code>xpacks/@xpack-dev-tools/bison/.content/bin</code>;
* add <code>@xpack-dev-tools/bison</code> to
  `package.json` as a
  development dependency; this associates a specific version of GNU bison with
...
it is also possible to install GNU bison only globally in
...
<CodeBlock language="sh"> {
`xpm install @xpack-dev-tools/bison@latest --global --verbose`
} </CodeBlock>
...
<CodeBlock language="sh"> {
`export PATH=$HOME/AppData/Roaming/xPacks/\@pack-dev-tools/bison /${ customField('upstreamVersion') }-${ customField('xpackSubversion') }.${ customField('npmSubversion') }/.content/bin:$PATH`
} </CodeBlock>
...
<CodeBlock language="sh"> {
`export PATH=$HOME/Library/xPacks/\@pack-dev-tools/bison /${ customField('upstreamVersion') }-${ customField('xpackSubversion') }.${ customField('npmSubversion') }/.content/bin:$PATH`
} </CodeBlock>
...
<CodeBlock language="sh"> {
`export PATH=$HOME/.local/xPacks/@xpack-dev-tools/bison /${ customField('upstreamVersion') }-${ customField('xpackSubversion') }.${ customField('npmSubversion') }/.content/bin:$PATH`
} </CodeBlock>
...
<CodeBlock language="sh"> {
`xpm uninstall @xpack-dev-tools/bison --verbose`
} </CodeBlock>

To completely remove the package from the central xPack store:

<CodeBlock language="sh"> {
`xpm uninstall --global @xpack-dev-tools/bison --verbose`
} </CodeBlock>
...
For all platforms, the **xPack GNU bison** binaries are released as portable
archives that can be installed in any location.

The archives can be downloaded from
the <a href={ `https://github.com/xpack-dev-tools/bison-xpack/releases/` }>GitHub Releases</a> pages.
...
The Windows versions of **xPack GNU bison**
are packed as `.zip` files.
Download the latest version named like:

* <code>xpack-bison-{ customField('upstreamVersion') }-{ customField('xpackSubversion') }-win32-x64.zip</code>
...
To manually install the <b>xPack GNU bison</b>,
unpack the archive and move it to a location of your choice.

The recommended location is
the <code>%USERPROFILE%\\AppData\\Roaming\\xPacks\\bison</code> folder, for
example <code>C:\\Users\\ilg\\AppData\\Roaming\\xPacks\\bison\\xpack-bison-{ customField('upstreamVersion') }-{ customField('xpackSubversion') }</code>.
...
* <code>xpack-bison-{ customField('upstreamVersion') }-{ customField('xpackSubversion') }-darwin-x64.tar.gz</code>
* <code>xpack-bison-{ customField('upstreamVersion') }-{ customField('xpackSubversion') }-darwin-arm64.tar.gz</code>
...
example <code>/Users/ilg/Library/xPacks/bison/xpack-bison-{ customField('upstreamVersion') }-{ customField('xpackSubversion') }</code>:

<CodeBlock language="console"> {
`mkdir -p ~/Library/xPacks/bison
cd ~/Library/xPacks/bison

tar xvf ~/Downloads/xpack-bison-${ customField('upstreamVersion') }-${ customField('xpackSubversion') }-darwin-x64.tar.gz
chmod -R -w xpack-bison-${ customField('upstreamVersion') }-${ customField('xpackSubversion') }`
} </CodeBlock>
...
example <code>/home/ilg/.local/xPacks/bison/xpack-bison-{ customField('upstreamVersion') }-{ customField('xpackSubversion') }</code>:

<CodeBlock language="console"> {
`mkdir -p ~/.local/xPacks/bison
cd ~/.local/xPacks/bison

tar xvf ~/Downloads/xpack-bison-${ customField('upstreamVersion') }-${ customField('xpackSubversion') }-linux-x64.tar.gz
chmod -R -w xpack-bison-${ customField('upstreamVersion') }-${ customField('xpackSubversion') }`
} </CodeBlock>
