...
# How to install the xPack GNU bison
...
[![GitHub Release Date](https://img.shields.io/github/release-date/xpack-dev-tools/bison-xpack)](https://github.com/xpack-dev-tools/bison-xpack/releases)
[![npm (scoped)](https://img.shields.io/npm/v/%40xpack-dev-tools%2Fbison?label=xpm&color=blue)](https://www.npmjs.com/package/@xpack-dev-tools/bison/)
![GitHub Downloads (all assets, all releases)](https://img.shields.io/github/downloads/xpack-dev-tools/bison-xpack/total)
[![npm](https://img.shields.io/npm/dm/%40xpack-dev-tools%2Fbison?label=xpm%20downloads)](https://www.npmjs.com/package/@xpack-dev-tools/bison/)
...
The easiest (and recommended) way to install xPack GNU bison
...
The command to install the **latest** available version of **bison** is:
...
<CodeBlock language="sh"> {
`xpm install @xpack-dev-tools/bison@3.8.2-1.1 --verbose
`} </CodeBlock>
...
* add <code>@xpack-dev-tools/bison</code> to
  `package.json` as a
  development dependency; this associates a specific version of xPack GNU bison with
...
it is also possible to install xPack GNU bison only globally in
...
In order to access the xPack GNU bison binaries installed in
...
<CodeBlock language="sh"> {
`export PATH=$HOME/Library/xPacks/\@pack-dev-tools/bison/3.8.2-1.1/.content/bin:$PATH
`} </CodeBlock>
...
<CodeBlock language="sh"> {
`export PATH=$HOME/.local/xPacks/@xpack-dev-tools/bison/3.8.2-1.1/.content/bin:$PATH
`} </CodeBlock>
...
<ul>
<li><code>xpack-bison-3.8.2-1-darwin-x64.tar.gz</code></li>
<li><code>xpack-bison-3.8.2-1-darwin-arm64.tar.gz</code></li>
</ul>
...
example <code>/Users/ilg/Library/xPacks/bison/xpack-bison-3.8.2-1</code>:

<CodeBlock language="console"> {
`mkdir -p ~/Library/xPacks/bison
cd ~/Library/xPacks/bison

tar xvf ~/Downloads/xpack-bison-3.8.2-1-darwin-x64.tar.gz
chmod -R -w xpack-bison-3.8.2-1
`} </CodeBlock>
...
<ul>
<li><code>xpack-bison-3.8.2-1-linux-x64.tar.gz</code></li>
<li><code>xpack-bison-3.8.2-1-linux-arm64.tar.gz</code></li>
<li><code>xpack-bison-3.8.2-1-linux-arm.tar.gz</code></li>
</ul>
...
example <code>/home/ilg/.local/xPacks/bison/xpack-bison-3.8.2-1</code>:

<CodeBlock language="console"> {
`mkdir -p ~/.local/xPacks/bison
cd ~/.local/xPacks/bison

tar xvf ~/Downloads/xpack-bison-3.8.2-1-linux-x64.tar.gz
chmod -R -w xpack-bison-3.8.2-1
`} </CodeBlock>
