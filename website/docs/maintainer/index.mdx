...
named like **xPack GNU bison v{customField('xpackVersion')}** (mind the dash),
...
<CodeBlock language="sh"> {
`npm dist-tag add @xpack-dev-tools/bison@${customField('xpackVersion')}.${customField('npmSubversion')} latest
`} </CodeBlock>
...
<CodeBlock language="sh"> {
`npm unpublish @xpack-dev-tools/bison@${customField('xpackVersion')}.${customField('npmSubversion')}
`} </CodeBlock>
