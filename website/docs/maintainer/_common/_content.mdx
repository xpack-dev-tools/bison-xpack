...
the **[Update the version specific code](http://localhost:3000/bison-xpack/docs/maintainer/#update-the-version-specific-code)** section.
...
and fix them; assign them to a milestone (like <code>3.8.2-1</code>).
...
- to remove the rest of the dates, change from:
  ...
  *
  ```
- add a new entry like _**\* v3.8.2-1 prepared**_
- **commit** with the message _**prepare v3.8.2-1**_
...
(like **3.8.2**) with the following expression:
...
[GitHub Actions](https://github.com/xpack-dev-tools/bison-xpack/actions) page.
...
```sh
xpm run trigger-workflow-test-prime -C ~/Work/xpack-dev-tools/bison-xpack.git/build-assets
xpm run trigger-workflow-test-docker-linux-x64 -C ~/Work/xpack-dev-tools/bison-xpack.git/build-assets
xpm run trigger-workflow-test-docker-linux-arm -C ~/Work/xpack-dev-tools/bison-xpack.git/build-assets
```
...
```sh
xpm run trigger-travis-macos -C ~/Work/xpack-dev-tools/bison-xpack.git/build-assets
```
...
git -C ~/Work/xpack-dev-tools/bison-xpack.git pull
xpm run install -C ~/Work/xpack-dev-tools/bison-xpack.git/build-assets
xpm run test-pre-release -C ~/Work/xpack-dev-tools/bison-xpack.git/build-assets
```
...
- run the **trigger-workflow-publish-release** xpm action:
  ```sh
  xpm run trigger-workflow-publish-release -C ~/Work/xpack-dev-tools/bison-xpack.git/build-assets
  ```
...
watching the bison-xpack project.
...
this will add a file in the `website/blog` folder:
  ```sh
  xpm run generate-website-blog-post -C ~/Work/xpack-dev-tools/bison-xpack.git/build-assets
  ```
...
- add entries in the **Bug fixes**; use the following syntax
  ```
  - [[#N](https://github.com/xpack-dev-tools/bison-xpack/issues/N)]:
  ```
- copy/paste the folders hierarchies from the install guide
- **commit** with the message _**website: blog post release 3.8.2-1 published**_
...
- run the **update-package-binaries** xpm action:
  ```sh
  xpm run update-package-binaries -C ~/Work/xpack-dev-tools/bison-xpack.git/build-assets
  ```
- open the top `package.json` file
...
- **commit** all changes with the message
  _**package.json: update urls for 3.8.2-1.1 release**_

### Publish on the npmjs.com server

- select the `xpack-development` branch
- check the latest commits `xpm run git-log`
- update `CHANGELOG.md`, add a line like _**\* v3.8.2-1.1 published on npmjs.com**_
- **commit** with the message _**CHANGELOG: publish npm v3.8.2-1.1**_
...
<a href="https://www.npmjs.com/package/@xpack-dev-tools/bison?activeTab=versions">npmjs.com</a>.
...
```sh
xpm run trigger-workflow-test-xpm -C ~/Work/xpack-dev-tools/bison-xpack.git/build-assets
```
...
- open https://xpack-dev-tools.github.io/bison-xpack/docs/maintainer/#publish-the-website in a separate window
...
in https://xpack-dev-tools.github.io/bison-xpack/blog/
...
```sh
xpm run trigger-workflow-deep-clean -C ~/Work/xpack-dev-tools/bison-xpack.git/build-assets
```
...
- paste the release name like _**xPack GNU bison v3.8.2-1 released**_
...
[![NPM Version](https://img.shields.io/npm/v/%40xpack-dev-tools%2Fbison?color=green)](https://www.npmjs.com/package/@xpack-dev-tools/bison/)
[![NPM Downloads](https://img.shields.io/npm/dm/%40xpack-dev-tools%2Fbison)](https://www.npmjs.com/package/@xpack-dev-tools/bison/)
  - all releases [![NPM Downloads](https://img.shields.io/npm/dt/%40xpack-dev-tools%2Fbison)](https://www.npmjs.com/package/@xpack-dev-tools/bison/)
